<script lang="ts">
	...
	import { updateEpubInDatabase } from '$lib/services/rebuild';
	import { removeSelectedEpub } from '$lib/services/utils';
	import { epubStore } from '$lib/store/epub.svelte';
	import { ArrowLeft } from '@lucide/svelte';

	let epub = $derived(epubStore.selectedEpub!);
...
<div class="h-full w-full lg:w-2/3 {epubStore.showMetadata ? 'flex flex-col' : 'hidden lg:block'}">
	{#if epubStore.selectedEpub}
		<div class="bg-base-100 flex w-full items-center border-b lg:hidden">
			<button
				onclick={removeSelectedEpub}
						...
						<div class="flex flex-col gap-6 lg:flex-row lg:gap-8">
										...
										{#if newCoverFile}
											<div class="text-center">
												<span class="text-success block text-xs"
													>Cover selected: {newCoverFile.name}</span
												>
												<span class="text-info text-xs"
													>Size: {sizeFormatter(newCoverFile.size)}</span
												>
											</div>
										{/if}
										...
										{#if (epub.coverBlob || newCoverFile) && !showResizeControls}
											<div class="text-center">
												<button
													...
													onclick={showResizeControlsForExisting}
												>
							...
							<div class="flex-1 space-y-4">
								...
								<div>
									...
									{:else}
										<h1
											class="text-primary text-2xl leading-tight font-bold sm:text-3xl lg:text-4xl"
										>
								...
								<div class="grid grid-cols-1 gap-3">
									{#each Object.entries(epub.metadata.identifiers) as [type, value]}
										...
										>
											...
											<span
												class="text-base-content text-right font-mono text-sm break-all select-all"
											>
												{value}
											</span>
										</div>
	...
	{:else if !epubStore.showMetadata}
